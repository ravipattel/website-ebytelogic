...
const DebounceComparison = ({ data }) => {
    ...
    return (
        <section id={comparisonData?.id} className="px-4 md:px-6 py-16 bg-slate-50">
            <h2 className="font-serif text-xl md:text-3xl font-semibold mb-12 text-center">
                            ...
                            <tbody>
                                {comparisonData?.quickReference?.rows?.map((row, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-3 px-4 text-gray-700 text-sm md:text-base">{row?.hardware}</td>
                                        <td className="py-3 px-4 text-gray-700 text-sm md:text-base">{row?.software}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    {comparisonData?.inDepthAnalysis?.map((item, idx) => (
                        <div key={idx} className="space-y-8">
                            <div>
                                <h4 className="font-serif text-xl font-semibold mb-4 flex flex-wrap items-center gap-3">
                                    {item?.icon}
                                    {item?.title}
                                </h4>
