    ...
    <section id="use-cases" className="px-4 md:px-6 py-16 bg-slate-50">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-gray-300 rounded-md p-4">
          ...
          <div className="space-y-4">
            {data?.useCase?.socApplications?.map((app, idx) => (
              <div className="flex items-start space-x-3" key={idx}>
                <div
                  className={`w-8 h-8 ${app.bgColor} rounded-full flex items-center justify-center flex-shrink-0 mt-1`}
                >
        ...
        <div className="bg-white border border-gray-300 rounded-md p-4">
          ...
          <div className="space-y-4">
            {data?.useCase?.somApplications?.map((app, idx) => (
              <div className="flex items-start space-x-3" key={idx}>
                <div
                  className={`w-8 h-8 ${app.bgColor} rounded-full flex items-center justify-center flex-shrink-0 mt-1`}
                >
      ...
      <div className="mt-12">
        <h3 className="text-2xl font-semibold mb-6">
          {data?.useCase?.blurringDistinction?.title}
        </h3>
        <div className="bg-white rounded-md p-6 border border-gray-300 space-y-3">
          <p className="text-base font-semibold">
            {data?.useCase?.blurringDistinction?.subtitle}
          </p>
          <p className="text-gray-700 mb-4 text-sm md:text-base">
            {data?.useCase?.blurringDistinction?.description}
            {data?.useCase?.blurringDistinction?.citation && (
              <a
                href={data?.blurringDistinction?.citation}
                className="text-primary hover:underline"
                target="_blank"
                rel="noopener noreferrer"
              >
                {[data?.blurringDistinction?.citationNo]}
              </a>
            )}
          </p>
          <div className="grid md:grid-cols-2 gap-4">
            {data?.useCase?.evolvingApplications?.map((item, idx) => {
              return (
                <div key={idx}>
                  <div className={`bg-${item.bgColor}-50 p-4 rounded-lg`}>
                    <h5 className={`font-semibold text-${item.iconColor}-900 mb-2`}>
                      {item.title}
                    </h5>
                    <p className={`text-sm text-${item.iconColor}-800`}>
                      {item.description}
                    </p>
                  </div>
                </div>
