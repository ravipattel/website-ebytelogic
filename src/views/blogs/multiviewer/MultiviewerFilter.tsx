...
const MultiviewerFilter = ({ data }) => {
    ...
    return (
        <section id={filterData?.id} className="px-4 md:px-6 py-16">
            <h2 className="font-serif text-2xl md:text-3xl font-semibold mb-8">{filterData?.title}</h2>
            <div className="bg-gradient-to-r from-gray-50 to-white rounded-xl p-8 mb-8 border-l-4 border-primary">
                <h3 className="font-serif text-xl md:text-2xl font-bold mb-4">{filterData?.architecture?.title}</h3>
                <p
                    className="text-gray-600 text-sm md:text-base mb-6"
                    dangerouslySetInnerHTML={{ __html: filterData?.architecture?.content }}
                />
                <div className="grid md:grid-cols-2 gap-8">
                    {filterData?.filterSections.map((section) => (
                        <div key={section.key}>
                            <h4 className="font-semibold text-base md:text-lg mb-3">{section.title}</h4>
            ...
            <div className="bg-white rounded-xl p-6 shadow-lg mb-8">
                <h4 className="font-serif text-xl md:text-2xl font-bold mb-4">{filterData?.implementation?.title}</h4>
                <p
                    className="text-gray-600 text-sm md:text-base mb-4"
                    dangerouslySetInnerHTML={{ __html: filterData?.implementation?.description }}
                />
